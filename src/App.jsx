  ...
  const [montantsManquants, setMontantsManquants] = useState(0);
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'PASSWORD_RECOVERY') {
        setResetMode(true); setResetSuccess(false); setUser(null); setLoading(false); done = true; return;
      }
      if (event === 'USER_UPDATED') {
        setResetMode(false); setResetSuccess(false);
        if (session?.user) { const u = await getCurrentUser(); setUser(u); } else { setUser(null); }
        setLoading(false); done = true; return;
      }
      if (event === 'SIGNED_IN') {
        if (session?.user) { const u = await getCurrentUser(); setUser(u); }
        setLoading(false); done = true; return;
      }
      if (event === 'SIGNED_OUT') { setUser(null); setLoading(false); done = true; return; }
    ...
    const fallback = setTimeout(() => {
      if (!done) { getCurrentUser().then(u => { setUser(u); setLoading(false); done = true; }); }
    }, 3000);
