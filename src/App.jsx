...
import Livrets       from './modules/Livrets';
import Facturation   from './modules/Facturation';
import Proprios      from './modules/Proprios';
import Tarification  from './modules/Tarification';
import AgentConfig   from './modules/AgentConfig';
import Deploiement   from './modules/Deploiement';
...
const injectGlobal = () => {
  ...
  s.textContent = `
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .res-block { border-radius:3px; padding:1px 5px; font-size:9px; font-family:'Montserrat',sans-serif; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; cursor:pointer; margin-bottom:2px; }
    .tab-btn { transition: all .15s; }
    .toggle-track { position:relative; width:32px; height:18px; background:#2A2A2A; border-radius:9px; transition:background .2s; display:inline-block; cursor:pointer; flex-shrink:0; }
    .toggle-track.on { background:#7A5E1A; }
    ...
    .conv-row:hover { background: #141414 !important; }
    .conv-row.active { background: #161410 !important; border-left-color: #C8A951 !important; }
    .step-btn { transition: all .15s; }
    .tpl-btn:hover { background: #3A2E1033 !important; color: #E2C97E !important; }
    .rule-row:hover { background: #161616 !important; }
  `;
...
const NAV = [
  ...
  { section:"Opérations", items:[
    ...
    { id:"templates",   icon:"✉", label:"Templates messages" },
  ]},
        ...
        <>
          ...
          {apparts.length > 0 ? (
            ...
            <div style={{ background:C.card, border:`0.5px solid ${C.borderGold}`, borderRadius:6, padding:'24px', textAlign:'center', marginBottom:20 }}>
              ...
              <div style={{ fontFamily:F.sans, fontSize:12, color:C.muted, lineHeight:1.7, marginBottom:16 }}>Votre base de données est prête. Commencez par ajouter votre premier appartement.</div>
              <button onClick={()=>setPage('appartements')}
                style={{ background:`linear-gradient(135deg,${C.goldDark},${C.gold})`, color:C.bg, border:'none', padding:'7px 16px', borderRadius:3, fontSize:10, fontWeight:600, cursor:'pointer', fontFamily:F.sans, letterSpacing:1, textTransform:'uppercase' }}>
  ...
  return (
    <div style={{ fontFamily:F.sans, background:C.bg, color:C.white, minHeight:'100vh', display:'flex', flexDirection:'column' }}>
      <div style={{ background:C.surface, borderBottom:`0.5px solid ${C.border}`, padding:'0 20px', height:54, display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:100, flexShrink:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <span style={{ fontFamily:F.sans, fontSize:10, color:C.muted }}><span style={{ color:C.successTxt }}>●</span> {role}</span>
      ...
      <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
        <div style={{ width:sidebarOpen?210:0, background:C.surface, borderRight:`0.5px solid ${C.border}`, display:'flex', flexDirection:'column', flexShrink:0, overflow:'hidden', transition:'width .2s ease' }}>
          <div style={{ flex:1, overflowY:'auto', padding:'14px 0' }}>
            {NAV.map((section, si) => (
              <div key={si}>
                <div style={{ fontFamily:F.sans, fontSize:8, letterSpacing:3, color:C.muted, textTransform:'uppercase', padding:'8px 18px 4px', fontWeight:600, marginTop:si>0?6:0 }}>{section.section}</div>
                {section.items.map(item => (
                  <div key={item.id} className={`nav-item${page===item.id?' active':''}`} onClick={() => setPage(item.id)}
                    ...
                    {item.badge && <span style={{ background:item.badgeColor||C.gold, color:C.bg, borderRadius:8, padding:'0 6px', fontSize:8, fontWeight:700 }}>{item.badge}</span>}
                  </div>
                ))}
              </div>
            ))}
